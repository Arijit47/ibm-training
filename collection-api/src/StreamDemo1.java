...
//		str1.forEach(System.out::println);
		...
		// Performing immediate operation
		...
		numbers.stream().distinct().sorted().forEach(System.out::println); // sorted() is intermediate operation
		...
		// Creating and working with streams and numbers
		System.out.println(IntStream.range(1, 99).count());
		System.out.println(IntStream.range(1, 9).sum());

		System.out.println(IntStream.range(1, 9).skip(5).sum()); // skip(5) means skip before 6 means skip 1,2,3,4,5
		System.out.println(IntStream.range(1, 9).skip(4).sum());
